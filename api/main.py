...
def main():
    ...
    for expmt_name in expmt_name_list:
        ...
        script_path = os.path.join(script_dir, 'run_expmt.py')
        
        if task == 'run':
            cmd = f'python "{script_path}" --n_samples {n_samples} --model {model}'
            if is_demo:
                cmd += ' --is_demo'
        elif task == 'calc':
            src_dir = os.path.join(os.path.dirname(script_dir), 'src')
            calc_metrics_path = os.path.join(src_dir, 'calc_metrics.py')
            cmd = f'python "{calc_metrics_path}" --n_samples 9999 --model {model}'
