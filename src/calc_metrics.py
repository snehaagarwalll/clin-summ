...
def main():
    ...
    # Load optional medical imaging metrics
    if HAS_RADGRAPH:
        f1radgraph = F1RadGraph(reward_level='partial')
    else:
        f1radgraph = None
        print("Warning: radgraph not installed. F1-RadGraph metric will be skipped.")
        
    if HAS_F1CHEXBERT and is_cxr:
        f1chexbert = F1CheXbert(device='cuda')
    else:
        f1chexbert = None
        if is_cxr and not HAS_F1CHEXBERT:
            print("Warning: f1chexbert not installed. F1-CheXbert metric will be skipped.")
            
    if HAS_UMLS:
        medcon = UMLSScorer()
    else:
        medcon = None
        print("Warning: UMLSScorer not available. MEDCON metric will be skipped.")
